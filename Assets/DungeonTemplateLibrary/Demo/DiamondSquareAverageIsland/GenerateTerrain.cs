...
public class GenerateTerrain : MonoBehaviour {
    ...
    void Start() {
        var matrix = new float[height + 1, width + 1];
        this.terrain = GetComponent<Terrain>();
        var terrainData = terrain.terrainData;
        SetTerrain(terrainData);
        GenerateDSATerrain(matrix);
    ...
    private void GenerateDSATerrain(float[,] matrix) {
        diamondSquaeAverageIsland = new DiamondSquareAverageIsland(minValue, altitude, addAltitude);
        diamondSquaeAverageIsland.DrawNormalize(matrix);
        Smooth(matrix, 2);
    }
